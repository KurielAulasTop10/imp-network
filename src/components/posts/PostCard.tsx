      ...
      <article className="mx-auto flex flex-col overflow-hidden transition-all duration-300 hover:scale-[1.05]">
        ...
        <div className="flex flex-col">
          <h3 className="text-lg md:text-lg font-bold mb-2 flex gap-3 items-start justify-start">
            <div className='w-1 h-6 bg-red-600' />{title}
          </h3>
          <p className="mb-4 text-sm text-gray-400 flex gap-1 uppercase font-semibold">
            <span className='text-red-600'>{author}</span> - {new Date(date.replace(/-/g, "/") + " 00:00:00").toLocaleDateString("pt-BR", { day: '2-digit', month: 'long', year: 'numeric' })} - <Link href={'/categoria/'+ categories[0]} className='text-red-600 hover:text-red-400'>{categories[0]}</Link>
          </p>
        </div>
      </article>
    </Link>
