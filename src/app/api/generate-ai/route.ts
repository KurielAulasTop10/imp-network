...
export async function POST(request: NextRequest) {
    const { title, article, slug } = await request.json();
    if (!title)
    ...
    const completionResume = await client.chat.completions.create({
        model: "gemini-2.0-flash-001",
        messages: [
            {
                role: "system",
    ...
    return NextResponse.json(
        {
            ...
            article: completionResume.choices[0].message.content,
        },
        { status: 200 },
    );
}
