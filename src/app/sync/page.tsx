...
export default function SyncPage() {
	...
	const sync = async (password: string) => {
		...
		const prevPostsRes = await fetch(
			`/api/posts/cache?password=${password}`,
		);
		const { posts: prevPosts }: { posts: Post[] } =
		...
		if (slugsToRevalidate.length === 0) {
			...
			promises.push(
				fetch(
					`/api/revalidate?path=/api/posts/cache&password=${password}`,
				),
			);
			promises.push(
			...
			slugsToRevalidate.forEach(slug => {
				promises.push(
					fetch(
						`/api/revalidate?path=/post/${slug}&password=${password}`,
					),
				);
			});
